...
def sce(delta):
    ...
    # super conductivity
    smt = np.zeros((N,), dtype=np.complex)
    for idx in range(N):
        ...
        smt[idx] = smt[idx] * g0 * k_step / (2. * np.pi)-1*delta[idx]
    return smt
# print(sce(0.05))
...
root=op.fsolve(sce,3*np.sin(np.arange(0,N)))
print(root)
# delta=3*np.sin(np.arange(0,N+1))
#
# print(sce(3*np.sin(np.arange(0,N+1))))
# print(root)
# plt.show()