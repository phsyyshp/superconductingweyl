...
for jj in range(np.size(n)):
    ...
    kstep2 = 0.1
    k1 = np.arange(-np.pi, np.pi, k_step)
    k2 = np.arange(-np.pi, np.pi, 2*kstep2)
    k3 = np.arange(-np.pi, np.pi, k_step)
    v=np.size(k1)*np.size(k2)*np.size(k3)
    kk, ws = sce(mu, N, k0, mg, k_step, kstep2, wd, 1)
    print(ws)
    print(np.size(kk)/3)
    ...
    plt.plot(n[jj],ws,'ko',markersize=3)
    tr=1./3.*((n[jj]+wd)**3-(n[jj]-wd)**3)*4*np.pi*v/(2*np.pi)**3/np.sin(k0)*4
    print(tr)
    plt.plot(n[jj],tr,'ro')

    plt.ylabel(r'$g(\mu)$',fontsize=25)
